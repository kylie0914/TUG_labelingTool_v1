...
class MainWindow (QMainWindow, mainGui):
    ...
    def changeName (self, path):
        for filename in os.listdir(path):
            if ".jpg" in filename:
                src = os.path.join(path, filename)
                tmpName = filename.split("_")
                if len(tmpName[1]) == 7:
                    dst = os.path.join(path, tmpName[0] +"_00" + tmpName[1])
                    os.rename(src, dst)
                elif len(tmpName[1]) == 8:
                    dst = os.path.join(path, tmpName[0] +"_0" + tmpName[1])
                    os.rename(src, dst)
        self.list_reset()
    ...
    def keyPressEvent(self, event):  # 
        ...
        if event.key() == Qt.Key_Escape:
            ...
            self.actionFunction()
            print( " [1] Start Move: ", self.imageIndexDisplay.text())

        elif event.key() == Qt.Key_2:
            self.startWalk_Btn.setChecked(True)
            self.actionFunction()
            print( " [2] Start Walking: " , self.imageIndexDisplay.text())

        elif event.key() == Qt.Key_3:
            self.startTurn_Btn.setChecked(True)
            self.actionFunction()
            print( " [3] Start Turning: " , self.imageIndexDisplay.text())
        elif event.key() == Qt.Key_4:
            self.endTurn_Btn.setChecked(True)
            self.actionFunction()
            print( " [4] End Turning: " , self.imageIndexDisplay.text())

        elif event.key() == Qt.Key_5:
            self.startSit_Btn.setChecked(True)
            self.actionFunction()
            print( " [5] Start Sitting: " , self.imageIndexDisplay.text())

        elif event.key() == Qt.Key_6:
            self.endSit_Btn.setChecked(True)
            self.actionFunction()
            print( " [6] End Sitting: " , self.imageIndexDisplay.text())
    ...
    def objectInit(self): 
        ...
        self.imageView.setGeometry( 50, 50, 1100, 800)
        ...
        if itemRow is not None:
            ...
            if "color_" in currentItem:
               # imageFile = self.directory.currentItem().text()
                ...
                pixmapResize = pixmap.scaled(1100,800, Qt.KeepAspectRatio)
                self.imageView.setPixmap(pixmapResize)
                self.imageView.show()
    ...
    def listItemClick(self):
        ...
        self.changeName(self.directory_path)
    ...
    def saveData(self): # saveBtn 클릭 시
        ...
        os.chdir(self.save_path)
        fileName = "label_" + dirList[3] + "_" + dirList[4] +".csv"
        ...
        with open(self.save_path +"/" + fileName, "a", encoding="utf-8") as f:
            ...
            f.write(data)
        # --- action image save (copy from dataset)
        for i in range(6):
            shutil.copyfile(self.directory_path + "/" + self.actionImagelist[i], self.save_path + "/" + self.actionImagelist[i])

        #  sekeleton file save
        for filename in os.listdir(self.directory_path):
            if ".csv" in filename:
                shutil.copyfile(self.directory_path + "/" + filename, self.save_path + "/skeleton_" + filename)
